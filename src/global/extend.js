...
function luckysheetextendtable (type, index, value, direction, sheetIndex) {
    ...
    //公式配置变动
    // 跨表引用问题修改 Mi
    luckysheetCoverSheetFormula(0, curOrder, file, index, value, direction);
    ...
    if (calcChain != null && calcChain.length > 0) {
        for (let i = 0; i < calcChain.length; i++) {
            ...
            if (calc_funcStr && calc_funcStr.indexOf('!') > -1) {
                continue;
            }
            if (type == "row") {
...
//处理跨表公式问题 MI
//controlType: 0-add, 1-del
function luckysheetCoverSheetFormula (controlType, curOrder, file, st, slen, direction) {
    for (let fIndex = 0; fIndex < Store.luckysheetfile.length; fIndex++) {
        if (fIndex == curOrder) {
            //只判断其他sheet 的引用，当前sheet 不用处理
            continue;
        }
        const lucFile = Store.luckysheetfile[fIndex];
        let dt = $.extend(true, [], lucFile.data);
        let tpCalcChain = lucFile.calcChain;
        if (tpCalcChain != null && tpCalcChain.length > 0) {
            for (let i = 0; i < tpCalcChain.length; i++) {
                let otherF = tpCalcChain[i];
                let calc_r = otherF.r, calc_c = otherF.c, calc_i = otherF.index, calc_funcStr = getcellFormula(calc_r, calc_c, calc_i);
                if (calc_funcStr.indexOf(file.name) >= 0) {
                    // console.log(otherF);
                    let functionStr = "";
                    if (controlType == 0) {
                        functionStr = "=" + formula.functionStrChange(calc_funcStr, "add", "row", direction, st, slen);

                    } else {
                        functionStr = "=" + formula.functionStrChange(calc_funcStr, "del", "row", null, st, slen);
                    }
                    dt[calc_r][calc_c].f = functionStr;
                }
            }
        }
        lucFile.data = dt;
...
function luckysheetdeletetable (type, st, ed, sheetIndex) {
    ...
    //公式配置变动
    // 跨表引用问题修改 Mi
    luckysheetCoverSheetFormula(1, curOrder, file, st, slen);
    let calcChain = file.calcChain;
    let newCalcChain = [];
    if (calcChain != null && calcChain.length > 0) {
        for (let i = 0; i < calcChain.length; i++) {
